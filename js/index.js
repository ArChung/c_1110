...
function initSBtn(){
    $('.sBtn').on('click',function(){
        ChungTool.pageScrollAni($("#"+$(this).attr('data-s')).offset().top-170)
    });

    $('.topBtn').on('click',function(){
        ChungTool.pageScrollAni($("#menu").offset().top)
    });
}

function initTopBtn() {
    window.onscroll = function () {
        if (ChungTool.windoePosTop() > 963) {
            $('.header').addClass('stick')
        } else {
            $('.header').removeClass('stick');
        }

        if (ChungTool.windoePosTop() > 1300) {
